...
def verify_dashboard(page: Page, base_url: str) -> None:
    ...
    history_payload = {
        ...
        "averages": {"temperature_c": 18.9, "humidity_pct": 51,
                     ...
                     "temperature_c_min": 10, "temperature_c_max": 30,
                     "humidity_pct_min": 20, "humidity_pct_max": 90,
                     "wind_kmh_min": 0, "wind_kmh_max": 16.09344,
                     "rain_mm_min": 0, "rain_mm_max": 25.4},
    }
    for field in ("temperature_c", "humidity_pct", "wind_kmh", "rain_mm"):
        history_payload["averages"][f"{field}_min_year"] = 1995
        history_payload["averages"][f"{field}_max_year"] = 2012
    ...
    expect(page.locator('[data-history-value="rain_mm"]')).to_have_text("Min 0.00 in (1995) Max 1.00 in (2012) Avg 0.08 in")
    ...
    expect(page.locator('[data-history-value="temperature_c"]')).to_have_text("Min 10.0°C (1995) Max 30.0°C (2012) Avg 18.9°C")
    expect(page.locator('[data-history-value="wind_kmh"]')).to_have_text("Min 0.0 km/h (1995) Max 16.1 km/h (2012) Avg 8.8 km/h")
    expect(page.locator('[data-history-value="rain_mm"]')).to_have_text("Min 0.0 mm (1995) Max 25.4 mm (2012) Avg 2.1 mm")
    history_payload["averages"]["rain_mm"] = 0
    history_payload["averages"]["humidity_pct"] = None
    history_payload["stale"] = True
    page.reload(wait_until="domcontentloaded")
    expect(page.locator('[data-history-value="rain_mm"]')).to_have_text("Min 0.0 mm (1995) Max 25.4 mm (2012) Avg 0.0 mm")
    expect(page.locator('[data-history-value="humidity_pct"]')).to_have_text("Min 20% (1995) Max 90% (2012) Avg —")
